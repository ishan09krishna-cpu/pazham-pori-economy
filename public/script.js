...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --------------------------------------------------------------------------
  // 12. 🏫 KERALA COLLEGE SIMULATOR CONTROLLER
  // --------------------------------------------------------------------------
  (function initCollegeSimulatorUI() {
    ...
    const simStartView = document.getElementById('simStartView');
    const simSetupView = document.getElementById('simSetupView');
    const simGameView = document.getElementById('simGameView');
    const simReportView = document.getElementById('simReportView');

    const startSimBtn = document.getElementById('startSimBtn');
    const archetypesGrid = document.getElementById('archetypesGrid');
    const randomArchetypeBtn = document.getElementById('randomArchetypeBtn');
    const confirmArchetypeBtn = document.getElementById('confirmArchetypeBtn');

    const simTimeBadge = document.getElementById('simTimeBadge');
    const simStepCounter = document.getElementById('simStepCounter');

    const stAttendance = document.getElementById('stAttendance');
    const stBrain = document.getElementById('stBrain');
    const stSleep = document.getElementById('stSleep');
    const stMoney = document.getElementById('stMoney');
    const stAssignments = document.getElementById('stAssignments');
    const stHappiness = document.getElementById('stHappiness');

    const simEventTitle = document.getElementById('simEventTitle');
    const simEventSubtitle = document.getElementById('simEventSubtitle');
    const simEventDialogue = document.getElementById('simEventDialogue');
    const simConsequenceBox = document.getElementById('simConsequenceBox');
    const simConsequenceText = document.getElementById('simConsequenceText');
    const simChoicesGrid = document.getElementById('simChoicesGrid');

    const repPlayerIcon = document.getElementById('repPlayerIcon');
    const repPlayerTitle = document.getElementById('repPlayerTitle');
    const repBadge = document.getElementById('repBadge');
    const repGrade = document.getElementById('repGrade');
    const repRemark = document.getElementById('repRemark');
    const simReplayBtn = document.getElementById('simReplayBtn');
    const simAskAIBtn = document.getElementById('simAskAIBtn');

    let selectedArchetypeId = null;

    if (!openSimBtn || !simModal) return;

    function showView(viewEl) {
      [simStartView, simSetupView, simGameView, simReportView].forEach(v => {
    ...
    // Start Button -> Go to Archetype Setup
    startSimBtn.addEventListener('click', () => {
      playSound('send');
      renderArchetypes();
      showView(simSetupView);
    });

    function renderArchetypes() {
      if (!window.CollegeSimulator) return;
      const archetypes = window.CollegeSimulator.getArchetypes();
      archetypesGrid.innerHTML = '';

      archetypes.forEach(arch => {
        const card = document.createElement('div');
        card.className = `archetype-card ${selectedArchetypeId === arch.id ? 'selected' : ''}`;
        card.innerHTML = `
          <div class="arch-head">
            <span class="arch-icon">${arch.icon}</span>
            <span class="arch-name">${arch.name}</span>
          </div>
          <div class="arch-tag">${arch.tagline}</div>
        `;
        card.addEventListener('click', () => {
          selectedArchetypeId = arch.id;
          confirmArchetypeBtn.disabled = false;
          renderArchetypes();
          playSound('receive');
        });
    ...
    randomArchetypeBtn.addEventListener('click', () => {
      ...
      selectedArchetypeId = randomArch.id;
      confirmArchetypeBtn.disabled = false;
      renderArchetypes();
      playSound('receive');
    });

    // Confirm Archetype -> Start Gameplay
    confirmArchetypeBtn.addEventListener('click', () => {
      if (!selectedArchetypeId || !window.CollegeSimulator) return;
      playSound('send');
      window.CollegeSimulator.initGame(selectedArchetypeId);
      updateGameUI();
      showView(simGameView);
    });

    // Update Live Game UI
    function updateGameUI() {
      const state = window.CollegeSimulator.getCurrentState();
      const events = window.CollegeSimulator.getEvents();
      const currentEvent = events[state.currentStepIndex];

      // Update Live Stats
      stAttendance.textContent = `${state.stats.attendance}%`;
      stBrain.textContent = state.stats.brainPower;
      stSleep.textContent = state.stats.sleep;
      stMoney.textContent = `₹${state.stats.money}`;
      stAssignments.textContent = state.stats.assignments;
      stHappiness.textContent = state.stats.happiness;

      if (!currentEvent) return;

      // Update Time & Event Details
      simTimeBadge.textContent = currentEvent.time;
      simStepCounter.textContent = `Step ${state.currentStepIndex + 1} of ${events.length}`;
      simEventTitle.textContent = currentEvent.title;
      simEventSubtitle.textContent = currentEvent.subtitle;
      simEventDialogue.textContent = currentEvent.dialogue;

      simConsequenceBox.classList.add('hidden');
      renderChoices(currentEvent.choices);
    }

    function renderChoices(choices) {
      simChoicesGrid.innerHTML = '';
      choices.forEach((c, idx) => {
        ...
        btn.addEventListener('click', () => {
          playSound('send');
          const result = window.CollegeSimulator.makeChoice(idx);
          if (result) {
            simConsequenceText.textContent = result.consequence;
            simConsequenceBox.classList.remove('hidden');

            setTimeout(() => {
              if (result.isFinished) {
                renderSurvivalReport();
                showView(simReportView);
              } else {
                updateGameUI();
              }
            }, 1200);
        ...
        simChoicesGrid.appendChild(btn);
      });
    }

    // Render Final Report
    function renderSurvivalReport() {
      if (!window.CollegeSimulator) return;
      const rep = window.CollegeSimulator.generateSurvivalReport();

      repPlayerIcon.textContent = rep.playerIcon;
      repPlayerTitle.textContent = rep.playerName;
      repBadge.textContent = rep.badge;
      repGrade.textContent = rep.grade;
      repRemark.textContent = `"${rep.remark}"`;
    }
    ...
    simReplayBtn.addEventListener('click', () => {
      ...
      confirmArchetypeBtn.disabled = true;
      showView(simStartView);
    });

    // Ask AI for Advice
    simAskAIBtn.addEventListener('click', () => {
      playSound('send');
      simModal.classList.add('hidden');
      const rep = window.CollegeSimulator.generateSurvivalReport();
      const promptText = `Da Kunjiraman, njan Kerala College Simulator kalichu! My final result: ${rep.title} (${rep.grade}). ${rep.remark}. Enikkum ini munnottu jeevikkan entha vazhi? 😂`;
